...
const asPath = item => typeof item === 'string' ? item : item.path

const throwIfBadFilepath = subject => {
  console.log(subject.item)
  const p = asPath(subject.item)
  if (p.indexOf('\\') > -1) {
...
const getPublicUrl = config => {
  const item = asPath(config.item)
  if (item.endsWith('assets') && config.packageName === 'govuk-frontend') {
    return '/govuk/assets'
  }
  return ['', 'extension-assets', config.packageName]
    .concat(item.split('/').filter(filterOutParentAndEmpty))
...
const getFileSystemPath = config => {
  ...
  return getPathFromProjectRoot('/node_modules',
    config.packageName,
    asPath(config.item).split('/').filter(filterOutParentAndEmpty).join(path.sep))
