...
module.exports = {
  ...
  getThresholds: async (id) => {
    ...
    FROM (
      SELECT t.id::text, w.ta_name AS description, t.targetarea_id,
      ...
      UNION (
        SELECT t.id::text, a.ta_name AS description, t.targetarea_id,
        CASE when t.stage = 'd' THEN concat(t.station_id, '-downstage') ELSE t.station_id END AS station_id,
    ...
    UNION (
      SELECT concat(rloi_id, '-max') AS id, null AS description, null AS targetarea_id, rloi_id AS station_id, 'max' AS type, level_max AS value, level_max_datetime AS date
      FROM measure_with_latest
      WHERE rloi_id = $1
    )
    UNION (
      SELECT concat(rloi_id, '-high') AS id, null AS description, null AS targetarea_id, rloi_id AS station_id, 'high' AS type, level_high AS value, null AS date
      FROM measure_with_latest
      WHERE rloi_id = $1
    )
    ORDER BY value desc;  
