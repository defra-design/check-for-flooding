...
module.exports = {
  ...
  getThresholds: async (id) => {
    ...
    SELECT * FROM (SELECT DISTINCT ON (targetarea_id) * FROM (
    ...
    'warning' AS type, t.type AS threshold, t.height::decimal AS value, null::timestamp AS date
    FROM trigger t
    ...
    ORDER BY targetarea_id, array_position(array['FW RES FW', 'FW ACT FW', 'FW ACTCON FW'], threshold)) wa
    UNION (
    SELECT * FROM (SELECT DISTINCT ON (targetarea_id) * FROM (
    SELECT t.id::text, s.name AS name, a.ta_name AS description, z.severity::integer, t.targetarea_id,
    CASE when t.stage = 'd' THEN concat(t.station_id, '-downstage') ELSE t.station_id END AS station_id,
    'alert' AS type, t.type AS threshold, t.height::decimal AS value, null::timestamp AS date
    FROM trigger t
    ...
    ORDER BY targetarea_id, array_position(array['FW RES FW', 'FW ACT FW', 'FW ACTCON FW'], threshold)) aa
    )
    UNION (
