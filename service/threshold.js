...
module.exports = {
  ...
  getThresholds: async (id) => {
    ...
    SELECT DISTINCT ON (targetarea_id) * FROM (
      SELECT t.id::text, s.name AS name, w.ta_name AS description, z.severity::integer, t.targetarea_id,
      CASE when t.stage = 'd' THEN concat(t.station_id, '-downstage') ELSE t.station_id END AS station_id,
      'warning' AS type, t.height::decimal AS value, null::timestamp AS date
      FROM trigger t
    ...
    UNION (SELECT * FROM (
      SELECT t.id::text, s.name AS name, a.ta_name AS description, z.severity::integer, t.targetarea_id,
      CASE when t.stage = 'd' THEN concat(t.station_id, '-downstage') ELSE t.station_id END AS station_id,
      'alert' AS type, t.height::decimal AS value, null::timestamp AS date
      FROM trigger t
    ...
    UNION (
      SELECT concat(rloi_id, '-max') AS id, null AS name, null AS description, null AS severity, null AS targetarea_id, rloi_id AS station_id, 'max' AS type, level_max::decimal AS value, level_max_datetime AS date
      FROM measure_with_latest
      WHERE rloi_id = $1
    )
    UNION (
      SELECT concat(rloi_id, '-high') AS id, null AS name, null AS description, null AS severity, null AS targetarea_id, rloi_id AS station_id, 'high' AS type, level_high::decimal AS value, null AS date
      FROM measure_with_latest
      WHERE rloi_id = $1
    )
    ORDER BY value desc;    
