...
module.exports = {
  getWarnings: async () => {
    ...
    SELECT lower(id) AS id, name, severity, message_changed_date AT TIME ZONE '+00' AS updated FROM warning
  ...
  getWarningsWithinBbox: async (bbox = []) => {
    ...
    `
    const queryBbox = `
      (SELECT lower(warning.id) AS id, warning.name, warning.severity AS severity, warning.message_changed_date AT TIME ZONE '+00' AS updated FROM warning
